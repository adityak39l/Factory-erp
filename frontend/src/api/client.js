...
export const endpoints = {
  ...
  employees: {
    ...
    salaryPreview: (id, params) => api.get(`/employees/${id}/salary-preview`, { params }),
    financialCalendar: (id, params) => api.get(`/employees/${id}/financial-calendar`, { params }),
    shorttimeLog: (id, params) => api.get(`/employees/${id}/shorttime-log`, { params }),
    salaryHistory: (id) => api.get(`/employees/${id}/salary-history`),
    updateSalary: (id, payload) => api.put(`/employees/${id}/salary`, payload),
    advances: (id) => api.get(`/employees/${id}/advances`),
  },
  payroll: {
    list: () => api.get('/payroll'),
    get: (id) => api.get(`/payroll/${id}`),
    calculate: (payload) => api.post('/payroll/calculate', payload),
    approve: (id) => api.put(`/payroll/${id}/approve`),
    markPaid: (id) => api.put(`/payroll/${id}/mark-paid`),
    addAdjustment: (id, empId, payload) => api.post(`/payroll/${id}/records/${empId}/adjustments`, payload),
    removeAdjustment: (id, empId, adjId) => api.delete(`/payroll/${id}/records/${empId}/adjustments/${adjId}`),
    updateOvertime: (id, empId, payload) => api.patch(`/payroll/${id}/records/${empId}/overtime`, payload),
    syncEmployees: (id) => api.post(`/payroll/${id}/sync`),
    exportExcel: (id) => downloadFile(`/payroll/${id}/export/excel`, {}, `Payroll_${id}.xlsx`),
    exportBank: (id) => downloadFile(`/payroll/${id}/export/bank`, {}, `Bank_Transfer_${id}.csv`),
    exportPayslip: (id, empId) => downloadFile(`/payroll/${id}/payslip/${empId}`, {}, `Payslip_${empId}.pdf`),
  },
  advances: {
    list: (params) => api.get('/advances', { params }),
    get: (id) => api.get(`/advances/${id}`),
    create: (payload) => api.post('/advances', payload),
    cancel: (id, payload) => api.patch(`/advances/${id}/cancel`, payload),
  },
  masters: {
